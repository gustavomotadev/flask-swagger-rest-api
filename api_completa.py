...
@doc(description='Obter lista de todos os autores.', tags=['Autor'])
@app.get('/v1/autores')
def get_autores():
    return repositorio.listar_todos_autores()

@doc(description='Obter um autor por ID.', tags=['Autor'])
@app.get('/v1/autor/<int:id>')
def get_autor_id(id):
    encontrado = repositorio.obter_autor_por_id(id)
    return encontrado if encontrado else ({'Erro': 'Não encontrado.'}, NOT_FOUND)

@doc(description='Obter um autor por Nome.', tags=['Autor'])
@app.get('/v1/autor/<string:nome>')
def get_autor_nome(nome):
...
@doc(description='Obter obras de um autor por ID.', tags=['Autor'])
@app.get('/v1/autor/<int:id>/obras')
def get_obras_autor_id(id):
    encontrado = repositorio.obter_autor_por_id(id)
    return repositorio.listar_obras_por_id_autor(id) if encontrado else ({'Erro': 'Não encontrado.'}, NOT_FOUND)

@doc(description='Obter obras de um autor por Nome.', tags=['Autor'])
@app.get('/v1/autor/<string:nome>/obras')
def get_obras_autor_nome(nome):
    encontrado = repositorio.obter_autor_por_nome(nome)
    return repositorio.listar_obras_por_nome_autor(nome) if encontrado else ({'Erro': 'Não encontrado.'}, NOT_FOUND)
...
@doc(description='Inserir um novo autor.', tags=['Autor'])
@use_kwargs({'Nome': fields.String(), 'AnoNascimento': fields.Integer(), 'AnoFalecimento': fields.Integer(), 'PaisOrigem': fields.String(), 'UrlImagem': fields.String()})
@app.post('/v1/autor')
def post_autor():
    try:
        nome = request.json['Nome']
        ano_nascimento = request.json['AnoNascimento']
        ano_falecimento = request.json['AnoFalecimento']
        pais_origem = request.json['PaisOrigem']
        url_imagem = request.json['UrlImagem']
        sucesso = repositorio.inserir_autor(nome, ano_nascimento, ano_falecimento, pais_origem, url_imagem)
        return ({'Mensagem': 'Inserido com sucesso.'}, CREATED) if sucesso else ({'Erro': 'Nome do Autor deve ser único.'}, BAD_REQUEST)
    except (TypeError,KeyError):
        return {'Erro': 'JSON enviado incorreto.'}, BAD_REQUEST
...
@doc(description='Editar um autor por ID.', tags=['Autor'])
@use_kwargs({'Nome': fields.String(), 'AnoNascimento': fields.Integer(), 'AnoFalecimento': fields.Integer(), 'PaisOrigem': fields.String(), 'UrlImagem': fields.String()})
@app.put('/v1/autor/<int:id>')
def put_autor_id(id):
    ...
    try:
        ...
        pais_origem = request.json['PaisOrigem']
        url_imagem = request.json['UrlImagem']
        sucesso = repositorio.editar_autor_por_id(id, nome, ano_nascimento, ano_falecimento, pais_origem, url_imagem)
        return {'Mensagem': 'Inserido com sucesso.'} if sucesso else ({'Erro': 'Nome do Autor deve ser único.'}, BAD_REQUEST)
    except (TypeError,KeyError):
        return {'Erro': 'JSON enviado incorreto.'}, BAD_REQUEST

@doc(description='Editar um autor por Nome.', tags=['Autor'])
@use_kwargs({'Nome': fields.String(), 'AnoNascimento': fields.Integer(), 'AnoFalecimento': fields.Integer(), 'PaisOrigem': fields.String(), 'UrlImagem': fields.String()})
@app.put('/v1/autor/<string:nome>')
def put_autor_nome(nome):
    ...
    try:
        ...
        pais_origem = request.json['PaisOrigem']
        url_imagem = request.json['UrlImagem']
        sucesso = repositorio.editar_autor_por_nome(nome, novo_nome, ano_nascimento, ano_falecimento, pais_origem, url_imagem)
        return {'Mensagem': 'Inserido com sucesso.'} if sucesso else ({'Erro': 'Nome deve ser único.'}, BAD_REQUEST)
    except (TypeError,KeyError):
        return {'Erro': 'JSON enviado incorreto.'}, BAD_REQUEST
...
@doc(description='Remover um autor por id.', tags=['Autor'])
@app.delete('/v1/autor/<int:id>')
def delete_autor_id(id):
    if not repositorio.obter_autor_por_id(id):
        return {'Erro': 'Não encontrado.'}, NOT_FOUND
    try:
        sucesso = repositorio.remover_autor_por_id(id)
        return {'Mensagem': 'Removido com sucesso.'} if sucesso else ({'Erro': 'Autor está sendo usado em obras.'}, CONFLICT)
    except (TypeError,KeyError):
        return {'Erro': 'JSON enviado incorreto.'}, BAD_REQUEST

@doc(description='Remover um autor por nome.', tags=['Autor'])
@app.delete('/v1/autor/<string:nome>')
def delete_autor_nome(nome):
    if not repositorio.obter_autor_por_nome(nome):
        return {'Erro': 'Não encontrado.'}, NOT_FOUND
    try:
...
@doc(description='Alterar URL da Imagem de um autor por ID.', tags=['Autor'])
@use_kwargs({'UrlImagem': fields.String()})
@app.patch('/v1/autor/<int:id>')
def patch_autor_url_id(id):
    try:
        url_imagem = request.json['UrlImagem']
        sucesso = repositorio.alterar_imagem_autor_por_id(id, url_imagem)
        return {'Mensagem': 'Inserido com sucesso.'} if sucesso else ({'Erro': 'Não Encontrado.'}, NOT_FOUND)
    except (TypeError,KeyError):
        return {'Erro': 'JSON enviado incorreto.'}, BAD_REQUEST

@doc(description='Alterar URL da Imagem de um autor por Nome.', tags=['Autor'])
@use_kwargs({'UrlImagem': fields.String()})
@app.patch('/v1/autor/<string:nome>')
def patch_autor_url_nome(nome):
    try:
        url_imagem = request.json['UrlImagem']
        sucesso = repositorio.alterar_imagem_autor_por_nome(nome, url_imagem)
        return {'Mensagem': 'Inserido com sucesso.'} if sucesso else ({'Erro': 'Não Encontrado.'}, NOT_FOUND)
    except (TypeError,KeyError):
        return {'Erro': 'JSON enviado incorreto.'}, BAD_REQUEST
